...
import { collection, getDocs, addDoc, doc, getDoc, updateDoc, query, where, Timestamp, runTransaction, writeBatch, deleteDoc, documentId, setDoc, limit, startAfter, orderBy, serverTimestamp, type Query, type DocumentSnapshot } from "@/lib/fs";
...
export const getInventoryMovements = async ({ page = 1, limit: itemsPerPage = 10, fetchAll = false, filters = {} }: { page?: number, limit?: number, fetchAll?: boolean, filters?: any } = {}): Promise<{ movements: InventoryMovement[], totalPages: number, totalCount: number }> => {
    ...
    if (matchingProductIds.length > 0) {
        const chunks = [];
        for (let i = 0; i < matchingProductIds.length; i += 30) {
            chunks.push(matchingProductIds.slice(i, i + 30));
        }
        movementsQuery = query(movementsQuery, where('productId', 'in', chunks[0]));
    }
...
export async function getDashboardRawData(filters: {
  ...
  const fetchWarehouseId = filters.warehouseId === 'wh-bog' ? undefined : filters.warehouseId;
  ...
  return { ordersResult, movementsResult, allProducts, allCategories, allPlatforms, allCarriers };
}
...
export async function getDashboardData(filters: { dateRange?: { from?: Date; to?: Date }; warehouseId?: string | null; platformIds: string[]; carrierIds: string[]; categoryIds: string[]; productIds: string[] }): Promise<DashboardData> {
  const raw = await getDashboardRawData({ dateRange: filters.dateRange, warehouseId: filters.warehouseId });
  return computeDashboardData(raw, filters);
}
